...
#include <boost/algorithm/string/split.hpp> //For read file. Using this algorithm take a lot of time.
...
int AdvancingFront::Discret(float rho = 0.1){ //Rho = number of points/volume unit
   ...
   //point initialisation :
      ...
      if (myfile.is_open()){
         ...
         point_given.push_back(First_Point); //Goes back to the first point
      }
      myfile.close();

   //***Discretisation***
   //We take point_given(2*number of initial points) and constitue Front with it :
   float d_rho = 1/sqrt(M_PI*rho); //d_rho is the characteristic distance in the mesh.
   for(int i = 0;i < point_given.size()-1;++i){
      ...
      for(int j=0; j < n+1 ; ++j){ //DEBUG problem : n+1 or n?
...
*/
void AdvancingFront::NewPoint(const float * seg, float ** return_new_point = nullptr){
   ...
   //cout<<"seg ="<<seg[0]<<" "<<seg[1]<<" "<<seg[2]<<" "<<seg[3]<<endl;   //DEBUG
   //With a rotation matrix, we can obtain :
   a[0] = (c[0]+b[0])*0.5 + (c[1]-b[1])*0.5*sqrt(3);
   a[1] = (b[0]-c[0])*0.5*sqrt(3) + (c[1]+b[1])*0.5;
   // We check if a neighbor is nearby.
   float closest_Front = CheckNeighbors(a, rho);
   if (closest_Front != -1){ //The new point not is an existing point. We check if there is a collision.
      ...
      }else{//We don't have a collision. a = existing point
      */
...
//We check if the new point causes a collision. If the new point is an existing point, we do not need to check.
}
...
float AdvancingFront::CheckNeighbors(float * a, float rho = 0.1){
   ...
   float d_rho = 1/sqrt(M_PI*rho);
   float r_test = d_rho/3; //r_test is the distance from the closest neighbor acceptable.1/2 of d_rho : Imperical value.
...
int AdvancingFront::Collision(float * a, float * b, float * c){
   ...
   while(!Mesh_copy.empty()){
      for (int k = 0; k<2; k++){
         ...
         //cout << "Mesh_copy.size()=" << Mesh_copy.size() <<endl; //DEBUG
...
int AdvancingFront::Intersect_segment(float * a,float * b, float * P){
   ...
   //f2(x) = A2*x + b2 = y
   if ((P[0]-P[2])==0){
      return 0; //DEBUG temporary solution
   }else if ((a[0]-b[0])==0){
      return 0;
   }
